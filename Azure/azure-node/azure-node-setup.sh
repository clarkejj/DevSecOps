...
ACR_NAME="wilson1"  # this must be unique among all others on Azure
...
RES_GROUP="b5cb3c0d-4813-4932-8929-031ff63e5846" # for premium
   # 281cf0b4-ecf5-4a80-b252-e9b90acfef8c
...
# For use in Bash (not Shell) from https://gist.github.com/cjus/1047794
function jsonValue() {
KEY=$1
num=$2
...
# Based on https://docs.microsoft.com/azure/container-registry/container-registry-tutorial-quick-task
# Based on https://docs.microsoft.com/azure/container-registry/container-registry-tutorial-build-task
...
# Sends the folder's contents to Azure Container Registry, which uses instructions in the Dockerfile to 
# build the "webimage" within and store it:
cd ~/mslearn-deploy-run-container-app-service
az acr build --registry "$ACR_NAME" --image webimage .
   # Expect "Run ID: cc1 was successful after 1m58s"
# TODO: Instead use az acr build-task per https://docs.microsoft.com/en-us/azure/container-registry/container-registry-tutorial-base-image-update#create-a-task
...
JSON_RESPONSE=$(az container create \
  --resource-group $RES_GROUP \
...
cat "$JSON_RESPONSE" | jsonValue "Started container" 1
exit
